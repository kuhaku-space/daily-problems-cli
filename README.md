...
```bash
daily login --server https://your-daily-problems.example.com   # APIトークンを保存
daily list                                                     # 公開中の問題一覧
...
daily submit 12 output.txt                                     # 問題の出力形式で正規化後に SHA-256 を計算して提出
daily submit output.txt                                        # 当日の問題に提出
```

- `login` は発行済みの API トークンを設定ファイルに保存します。`--token` を省略するとプロンプトで尋ねます。
  トークンは Daily Problems のプロフィールページ「API トークン」から発行してください（ユーザー名・パスワードによる発行は廃止されました）。
- `get` / `download` / `dl` / `submit` は問題番号を省略すると、当日の問題を対象にします。
- `submit` は問題に設定された出力フォーマッタで正規化してから SHA-256 を計算して送ります。正解(AC)なら終了コード 0、不正解(WA)なら 1。
...
- `--output-formatter` で想定出力の正規化方式を指定できます。`identity-v1`（完全一致）、`newlines-v1`（改行コードのみ無視）、`tokens-v1`（空白区切り）、`tokens-case-insensitive-v1`、`yesno-v1` をサポートします。フォーマッタを変更する `edit` では、`--answer` または `--answer-sha256` も必要です。
